...
curl -s "https://review.rdoproject.org/repoxplorer/api/v1/tops/authors/diff?pid=RDO&limit=1000&dfrom=$FROM&dto=$TO&dfromref=2018-04-10&dtoref=$FROM" | jq|grep name |sort -u|awk -F: '{print $2}'| tr -d '"'
else
echo -e "The full list of contributors in the period $FROM to $TO:"
curl -s "https://review.rdoproject.org/repoxplorer/api/v1/tops/authors/bycommits?pid=RDO&limit=1000&dfrom=$FROM&dto=$TO"|jq|grep name|sort -u|awk -F: '{print $2}'| tr -d '"'
fi
