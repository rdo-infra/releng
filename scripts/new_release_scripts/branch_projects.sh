...
function help(){
    ...
    echo "fix - to branch own list of projects"
    echo
...
os-refresh-config"
        ...
        #sed -i 's/-ui/-dashboard/' "$BRANCHED_PROJECTS_FILE"
        ...
        # here anditional projects to branch can be specified
        PROJECT_LIST=""
        echo "$PROJECT_LIST" > "$BRANCHED_PROJECTS_FILE"
    else
        echo "ERROR: Provided argument doesn't match to any phase."
        exit 1
    fi
}
