...
if [[ -d "$RDOINFO_DIR" ]]; then
    ...
    git clone git@github.com:redhat-openstack/rdoinfo.git $TMPDIR/rdoinfo
    if [[ $? -eq 0 ]]; then
...
pushd $RDOINFO_DIR >/dev/null 2>&1
git commit -am "Pin non-openstack puppet modules for ${MASTER_RELEASE^}"
popd >/dev/null
