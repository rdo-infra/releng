...
LAST_NVR=$(rdopkg findpkg $PKG| grep cloud9s-openstack-${LATEST_RELEASE}-testing|awk '{print $2}')
echo "Last release in ${LATEST_RELEASE}: $LAST_NVR"

NEWCOMMIT=$(rdopkg findpkg $PKG|grep -A1 ${MASTER_RELEASE_TAG}|grep source-branch|awk '{print $2}')
