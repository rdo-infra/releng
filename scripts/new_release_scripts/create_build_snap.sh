...
LASTRELEASE=$(rdopkg info $PKG| grep cloud7-openstack-${PREV_RELEASE}-testing|awk '{print $2}')
echo "Last release in pike: $LASTRELEASE"

NEWCOMMIT=$(rdopkg info $PKG|grep -A1 $RELEASE|grep source-branch|awk '{print $2}')
...
if [ $NEWCOMMIT == $CURRENTCOMMIT ]; then
    ...
    echo "cbs tag-build cloud7-openstack-$RELEASE-candidate $LASTRELEASE"
    read -n 2
    cbs add-pkg cloud7-openstack-$RELEASE-candidate $PKG --owner=amoralej
    cbs tag-build cloud7-openstack-$RELEASE-candidate $LASTRELEASE
    exit 0
fi
...
CHANGELOG="* Thu Feb 15 2018 RDO <dev@lists.rdoproject.org> ${NEWVERSION}-${NEWRELEASE}.$(echo $NEWCOMMIT|cut -c -7)git\n- Update to post $NEWVERSION \($NEWCOMMIT\)\n"
