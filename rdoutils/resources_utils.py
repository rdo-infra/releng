...
def branch_dependency(dep_name, new_branch, start_point, local_config):
    ...
    update_dep_default_branch(resource_filename, dep_name, new_branch, local_config)

def branch_dependencies_from_file(deps_file, new_branch, start_point, local_config):
    try:
        f = open(deps_file, "r")
        deps = f.readlines()
    except IOError:
