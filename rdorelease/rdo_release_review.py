...
def process_package(name, version, osp_release, dry_run, check_tag=False,
    ...
    try:
        ...
        if not is_newer(new_evr, old_evr):
            ...
            return
        if check_tarball and not wait_for_tarball(package.name):
            tag_exists = is_release_tag(package.name, package.version)
            log_message('INFO', "Tarball for %s %s is not ready yet, Tag exists: %s" %
                        (package.name, package.version, tag_exists), logfile)
